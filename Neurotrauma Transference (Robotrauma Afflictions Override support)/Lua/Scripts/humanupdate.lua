...
NTRT.RemovedAfflictions = { -- afflictions that are periodically removed from robots
    "huskinfection", "bleeding", "bleedingnonstop", "nausea", "organdamage", "psychosis", "concussion", "bloodloss", "acidburn", "cardiacarrest", "respiratoryarrest", "pneumothorax", "tamponade", "heartattack", "heartremoved", "lungremoved", "liverremoved", "kidneyremoved", "cerebralhypoxia", "heartdamage", "lungdamage", "liverdamage", "kidneydamage", "bonedamage", "organdamage", "sepsis", "immunity", "bloodpressure", "hypoxemia", "hemotransfusionshock", "internalbleeding", "acidosis", "alkalosis", "stroke", "coma", "t_paralysis", "ll_fracture", "rl_fracture", "ra_fracture", "la_fracture", "h_fracture", "n_fracture", "t_fracture", "ll_arterialcut", "rl_arterialcut", "ra_arterialcut", "la_arterialcut", "h_arterialcut", "t_arterialcut", -- theres so many things that a robot shouldnt have...
    "anesthesia", "drunk", -- this one might be fun to not disable, your choice
    "afadrenaline", "afadrenaline", "afantibiotics", "afthiamine", "afsaline", "afringerssolution", "afstreptokinase", "afmannitol", "afpressuredrug"
}
...
local limbtypes = {LimbType.Torso, LimbType.Head, LimbType.LeftArm, LimbType.RightArm, LimbType.LeftLeg, LimbType.RightLeg}
...
Timer.Wait(function()
    ...
    NT.UpdateHuman = function(character)
        ...
        for key, val in pairs(NTC.PreHumanUpdateHooks) do val(character) end

        local TableContains = function(table, identifier) return table[identifier] ~= nil end
        ...
        for identifier, data in pairs(NT.CharStats) do
            if not charIsRobot or TableContains(NTRT.CharStats, identifier) then
                if data.getter ~= nil and (not charIsRobot or NTRT.CharStats[identifier].leaveempty == nil) then
        ...
        for identifier, data in pairs(NT.Afflictions) do if not charIsRobot or (TableContains(NTRT.Afflictions, identifier) and NTRT.Afflictions[identifier].getonly == nil) then if data.update ~= nil then data.update(charData, identifier) end end end
        ...
        local function UpdateLimb(type)
            local keystring = tostring(type) .. "afflictions"
            for identifier, data in pairs(NT.LimbAfflictions) do if not charIsRobot or (TableContains(NTRT.LimbAfflictions, identifier) and NTRT.LimbAfflictions[identifier].getonly == nil) then if data.update ~= nil then data.update(charData, charData[keystring], identifier, type) end end end
        end
        local function ApplyLimb(type)
            local keystring = tostring(type) .. "afflictions"
            for identifier, data in pairs(charData[keystring]) do
                local newval = HF.Clamp(data.strength, NT.LimbAfflictions[identifier].min or 0, NT.LimbAfflictions[identifier].max or 100)
        ...
        if not charData.stats.stasis then
            for type in limbtypes do FetchLimbData(type) end
            for type in limbtypes do UpdateLimb(type) end
            for type in limbtypes do ApplyLimb(type) end
        end

        -- non-limb-specific late update (useful for things that use stats that are altered by limb specifics)
        for identifier, data in pairs(NT.Afflictions) do if not charIsRobot or TableContains(NTRT.Afflictions, identifier) then if data.lateupdate ~= nil then data.lateupdate(charData, identifier) end end end

        -- apply non-limb-specific changes
        for identifier, data in pairs(charData.afflictions) do
            local newval = HF.Clamp(data.strength, NT.Afflictions[identifier].min or 0, NT.Afflictions[identifier].max or 100)
            if newval ~= data.prev then
        ...
        -- humanupdate hooks
        for key, val in pairs(NTC.HumanUpdateHooks) do val(character) end
...
function NTRT.UpdateRobot(character)
    ...
    if HF.GetAfflictionStrength(character, "robotspawned") < 0.5 then
        ...
        for limbtype in limbtypes do NTCyb.CyberifyLimb(character, limbtype) end

        HF.SetAffliction(character, "robotspawned", 100)
    end

    -- remove fleshy afflictions
    for identifier in NTRT.RemovedAfflictions do if HF.GetAfflictionStrength(character, identifier) > 0 then HF.SetAffliction(character, identifier, 0) end end
end
...
Timer.Wait(function()
    ...
    for func in NTC.PreHumanUpdateHooks do
        if func == NTCyb.UpdateHuman then removeIndex = i end
        i = i + 1
    end
...
function NTRT.UpdateHuman(character)
    ...
    local velocity = 0
    if character ~= nil and character.AnimController ~= nil and character.AnimController.MainLimb ~= nil and character.AnimController.MainLimb.body ~= nil and character.AnimController.MainLimb.body.LinearVelocity ~= nil then velocity = character.AnimController.MainLimb.body.LinearVelocity.Length() end
    ...
    local function updateLimb(character, limbtype)
        ...
        if bentmetal > 5 and (limbtype == LimbType.LeftLeg or limbtype == LimbType.RightLeg) then NTC.MultiplySpeed(character, 1 - (bentmetal / 100) * 0.5) end
    end
...
function NTRT.ConvertDamageTypes(character, limbtype)
    ...
    if isRobot or NTCyb.HF.LimbIsCyber(character, limbtype) then
        ...
        local acidburn = HF.GetAfflictionStrengthLimb(character, limbtype, "acidburn", 0)
        ...
        loosescrews = loosescrews + 1 * (0.25 * damageChance(lacerations, 0.75) + 1 * damageChance(explosiondamage, 0.8) + 0.5 * damageChance(blunttrauma, 0.5) + 1 * damageChance(internaldamage, 0.75) + 0.5 * damageChance(bitewounds, 0.5) + 0.75 * damageChance(foreignbody, 0.75))

        damagedelectronics = damagedelectronics + 0.5 * (1 + prevmaterialloss / 50) * (2 * damageChance(burn, 0.75) + 0.75 * damageChance(gunshotwound, 0.85) + 0.25 * damageChance(bitewounds, 0.5) + 0.5 * damageChance(explosiondamage, 0.5) + 1 * damageChance(blunttrauma, 0.5) + 1 * damageChance(internaldamage, 0.75) + 0.75 * damageChance(foreignbody, 0.75))

        bentmetal = bentmetal + 1 * (0.25 * damageChance(burn, 0.85) + 0.25 * damageChance(lacerations, 0.5) + 0.5 * damageChance(bitewounds, 0.5) + 1 * damageChance(explosiondamage, 0.85) + 2 * damageChance(blunttrauma, 0.75))

        materialloss = materialloss + (1 + prevloosescrews / 50) * (0.85 * damageChance(acidburn, 0.85) + 0.5 * damageChance(lacerations, 0.75) + 0.8 * damageChance(gunshotwound, 0.8) + 0.6 * damageChance(bitewounds, 0.75) + 1 * explosiondamage + 0.5 * damageChance(foreignbody, 0.8))
        ...
        HF.ApplyAfflictionChangeLimb(character, limbtype, "acidburn", 0, acidburn, 0, 100)
